...
def test_state_engine_emits_canonical(tmp_path):
    ...
    assert canonical["leds"], "Expected at least one LED in canonical state"
    led_entry = canonical["leds"][0]
    assert led_entry["name"] == "Test LED"
    assert "health" in led_entry
    assert "activity_level" in led_entry
    assert canonical.get("context", {}).get("daypart") == "morning"
